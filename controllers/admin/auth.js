...
const ADMIIN = require("../../models/Admin");
...
exports.signup = async (req, res, next) => {
  try {
    ...
    const existingUser = await ADMIIN.findOne({
      $or: [{ username }, { email }],
    });
    if (existingUser) {
...
exports.signin = async (req, res, next) => {
  try {
    ...
    const user = await ADMIIN.findOne({ email });
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    let user = await ADMIIN.findOneAndUpdate(
...
exports.verifyOTP = async (req, res) => {
  try {
    const { otp } = req.body;
    const data = await ADMIIN.findOne({ email: req.body.email });
...
exports.changePassword = async (req, res) => {
  try {
    ...
    const user = await ADMIIN.findOne({ email: req.body.email });
    const hashedPassword = await bcrypt.hash(password, 10);
...
exports.refreshToken = async (req, res) => {
  try {
    const accessToken = generateAccessToken(req.userId, "admin");
